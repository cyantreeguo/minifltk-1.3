		...
		case EEventKey:
			if (0) // keyCode == ' ') // keyCode)
				...
				Fl::e_text = buffer;
					...
					if ((kev.iCode >= '0' && kev.iCode <= '9') || kev.iCode == '*' || kev.iCode == '#') // || kev.iCode == ' ')
					{
					buffer[0] = kev.iCode;
					buffer[1] = 0;
...
int fl_wait(double time_to_wait)
	...
	if (fl_symbianevent_mask == FL_SYMBIANEVENT_WSREDRAW)
		...
		if (!redrawEvent.Handle())
			...
			if (fl_symbianevent_mask == FL_SYMBIANEVENT_WSEVENT)
				{
				if (!wsEvent.Handle())
						{
						fl_s60_wait_result_ = 0;
				...
				if (wsEvent.Type() == EEventPointer)
					...
					if (wsEvent.Type() == EEventKey || wsEvent.Type() == EEventKeyUp || wsEvent.Type() == EEventKeyDown)
						{
							fl_s60_wait_result_ = key_event (w, wsEvent);
						} else
							if (wsEvent.Type() == EEventFocusGained)
								fl_s60_wait_result_ = Fl::handle(FL_FOCUS, w);
							else
								if (wsEvent.Type() == EEventFocusLost)
									fl_s60_wait_result_ = Fl::handle(FL_UNFOCUS, w);
				} else
					if (fl_symbianevent_mask == FL_SYMBIANEVENT_WAITTIMEOUT)
						{
						fl_s60_wait_result_ = 0;
						} else
							if (fl_symbianevent_mask == FL_SYMBIANEVENT_USERTIMEOUT)
								{
...
/* int fl_wait(double time_to_wait)
	...
	if (redrawReadyStatus != KRequestPending)
		...
		if (eventReadyStatus != KRequestPending)
			{
			
			} else
				if (time_to_wait < 2100 && timeoutStatus != KRequestPending)
					{
					return 0;
					}
	TInt schedulerError;
